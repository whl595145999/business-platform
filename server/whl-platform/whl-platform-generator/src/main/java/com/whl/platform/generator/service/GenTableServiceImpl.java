package com.whl.platform.generator.service;
...
import com.whl.framework.core.utils.file.FileUtils;
...
import com.whl.platform.generator.mapper.GenTableColumnMapper;
import com.whl.platform.generator.mapper.GenTableMapper;
import com.whl.platform.generator.util.GenUtils;
import com.whl.platform.generator.util.VelocityInitializer;
import com.whl.platform.generator.util.VelocityUtils;
...
public class GenTableServiceImpl implements IGenTableService {
    ...
    public void generatorCode(Long tableId) {
        ...
        // 获取模板列表
        List<String> templates = VelocityUtils.getTemplateList(table.getTplCategory(), table.getDataName());
        boolean scmCrud = GenTable.isScmCrud(table.getTplCategory());
        for (String template : templates) {
            if (!scmCrud && StringUtils.containsAny(template, "sql.vm", "api.ts.vm", "types.ts.vm", "index.vue.vm", "index-tree.vue.vm")) {
                continue;
            }
            StringWriter sw = new StringWriter();
            Template tpl = Velocity.getTemplate(template, Constants.UTF8);
    ...
    public void validateEdit(GenTable genTable) {
        ...
        } else if (GenConstants.TPL_SCM_CRUD.equals(genTable.getTplCategory())) {
            String options = JsonUtils.toJsonString(genTable.getParams());
            Dict paramsObj = JsonUtils.parseMap(options);
            if (StringUtils.isEmpty(paramsObj.getStr(GenConstants.SCM_DOMAIN))) {
                throw new ServiceException("SCM 域不能为空");
            } else if (StringUtils.isEmpty(paramsObj.getStr(GenConstants.SCM_RESOURCE))) {
                throw new ServiceException("SCM REST 资源名不能为空");
            } else if (StringUtils.isEmpty(paramsObj.getStr(GenConstants.SCM_MENU_ID))) {
                throw new ServiceException("SCM 菜单 ID 不能为空");
            }
        }
    }
