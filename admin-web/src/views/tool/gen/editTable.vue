      ...
      <el-tab-pane label="字段信息" name="columnInfo">
        <el-table ref="dragTable" border :data="columns" row-key="columnId" :max-height="tableHeight">
          ...
          <el-table-column label="主键" min-width="5%">
            <template #default="scope">
              <el-checkbox v-model="scope.row.isPk" true-value="1" false-value="0" disabled></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column label="自增" min-width="5%">
            <template #default="scope">
              <el-checkbox v-model="scope.row.isIncrement" true-value="1" false-value="0"></el-checkbox>
            </template>
          </el-table-column>
          ...
          <el-table-column label="显示类型" min-width="12%">
            <template #default="scope">
              <el-select v-model="scope.row.htmlType">
                ...
                <el-option label="部门树" value="deptTree" />
                <el-option label="组织下拉" value="orgSelect" />
                <el-option label="租户下拉" value="tenantSelect" />
              </el-select>
            </template>
          </el-table-column>
  ...
  Promise.all([basicForm, genForm].map(getFormPromise)).then(async (res) => {
    ...
    if (validateResult) {
      ...
      genTable.params = {
        ...
        scmDomain: info.value?.scmDomain,
        scmResource: info.value?.scmResource,
        scmBusinessKey: info.value?.scmBusinessKey,
        scmMenuId: info.value?.scmMenuId,
        scmMenuOrder: info.value?.scmMenuOrder,
        scmMenuComponent: info.value?.scmMenuComponent,
        scmPermPrefix: info.value?.scmPermPrefix,
        scmGenerateOptions: info.value?.scmGenerateOptions
      };
...
(async () => {
  ...
  if (tableId) {
    ...
    if (info.value?.options) {
      try {
        const opts = typeof info.value.options === 'string' ? JSON.parse(info.value.options) : info.value.options;
        info.value.scmDomain = opts.scmDomain;
        info.value.scmResource = opts.scmResource;
        info.value.scmBusinessKey = opts.scmBusinessKey;
        info.value.scmMenuId = opts.scmMenuId;
        info.value.scmMenuOrder = opts.scmMenuOrder;
        info.value.scmMenuComponent = opts.scmMenuComponent;
        info.value.scmPermPrefix = opts.scmPermPrefix;
        info.value.scmGenerateOptions = opts.scmGenerateOptions !== false;
        if (opts.parentMenuId) {
          info.value.parentMenuId = opts.parentMenuId;
        }
      } catch {
