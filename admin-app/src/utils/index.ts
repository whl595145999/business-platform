...
export function getEnvBaseUrl() {
  ...
  if (isMpWeixin) {
    ...
    // 真机开发版若仍指向 localhost，请求会打到手机自身导致 ERR_CONNECTION_REFUSED
    if (
      import.meta.env.DEV
      && envVersion === 'develop'
      ...
      console.warn(
        '[API] 微信小程序真机无法访问 localhost。请复制 env/.env.development.local.example 为 .env.development.local，'
        + '设置 VITE_SERVER_BASEURL__WEIXIN_DEVELOP 为你电脑的局域网 IP（如 http://192.168.1.100:8080），然后重新 pnpm dev:mp',
      )
    }
  }
...
/** 是否为业务平台后端认证模式（对接 admin-web / RuoYi-Vue-Plus） */
export const isPlatformAuthMode = import.meta.env.VITE_AUTH_MODE === 'platform'
